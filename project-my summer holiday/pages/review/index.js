...
Page({
 data:{id:'',chapter:null,entries:[],limit:8,hasMore:false,missing:false,composing:false,ending:'',generating:false,posterPath:'',error:'',expanded:false,coverFailed:false,featured:[]},
 ...
 async generatePoster(){
  ...
  try{
   ...
   ctx.setFillStyle('#F9FAFC');ctx.fillRect(0,0,750,1100);ctx.setFillStyle('#293246')
   ...
   ctx.setFillStyle('#536C9B');ctx.setFontSize(23);ctx.fillText(chapter.startDate+' — '+(chapter.status==='ONGOING'?'进行中':chapter.endDate||'已结束'),48,y+12);y+=52
   ...
   ctx.setFillStyle('#536C9B');ctx.setFontSize(21);ctx.fillText(chapter.status==='ONGOING'?'进行中':'已结束',48,1052)
