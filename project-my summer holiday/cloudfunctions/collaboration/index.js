...
const tables = { users:'ongoing_users',chapters:'ongoing_chapters',moments:'ongoing_moments',contributions:'ongoing_contributions',comments:'ongoing_comments',invites:'ongoing_invites' }
...
async function versioned(table,item,existing,operationId,baseVersion) {
  ...
  else { try{await db.collection(tables[table]).add({data:Object.assign({_id:item.id},fields)})}catch(e){const current=await one(table,item.id);if(current&&current.lastOperation===operationId)return clean(current);if(current)fail('CONFLICT');throw e} }
...
async function momentSnapshot(id,user) { const moment=await requireMoment(id,user);const rows=await list('contributions',{momentId:id});const comments=await list('comments',{momentId:id});const contributions=rows.filter(x=>!x.deleted);const visibleComments=comments.filter(x=>!x.deleted);return withMedia({moment:clean(moment),contributions:clean(contributions),comments:clean(visibleComments),deletedIds:rows.concat(comments).filter(x=>x.deleted).map(x=>x.id),users:await usersFor([moment.creatorId,...contributions.map(x=>x.creatorId),...visibleComments.map(x=>x.creatorId)])}) }
async function chapterSnapshot(id,user) { const chapter=await requireChapter(id,user);const all=await list('moments',{chapterId:id});const moments=all.filter(x=>!x.deleted&&x.status!=='DRAFT');const ids=new Set(moments.map(x=>x.id));const rows=await list('contributions',{chapterId:id});const comments=await list('comments',{chapterId:id});const contributions=rows.filter(x=>!x.deleted&&ids.has(x.momentId));const visibleComments=comments.filter(x=>!x.deleted&&ids.has(x.momentId));return withMedia({chapter:clean(chapter),moments:clean(moments),contributions:clean(contributions),comments:clean(visibleComments),deletedIds:all.concat(rows,comments).filter(x=>x.deleted).map(x=>x.id),users:await usersFor([...(chapter.memberIds||[]),...moments.map(x=>x.creatorId),...contributions.map(x=>x.creatorId),...visibleComments.map(x=>x.creatorId)])}) }
async function ensureUser(user,profile,update) {
 const existing=await one('users',user)
 if(existing&&!update&&existing.nickname&&existing.nickname!=='我')return clean(existing)
 const value=pick(profile||{},['nickname','avatar','bio'])
 value.nickname=String(value.nickname||existing&&existing.nickname||'').trim().slice(0,40)
 if(!value.nickname||value.nickname==='我')value.nickname='记录者·'+crypto.createHash('sha256').update(user).digest('hex').slice(0,4)
 try{return clean(await write('users',user,Object.assign({id:user},value),existing))}
 catch(error){
  // Two devices signing into the same account may create its profile at the same time.
  if(!existing&&!update){const created=await one('users',user);if(created)return clean(created)}
  throw error
 }
}
...
async function saveComment(user,event) { const input=event.comment||{};const m=await requireMoment(input.momentId,user);const perspective=await one('contributions',input.perspectiveId);if(!perspective||perspective.deleted||perspective.momentId!==m.id)fail('PERSPECTIVE_NOT_FOUND');const existing=await one('comments',input.id);if(existing&&(existing.creatorId!==user||existing.perspectiveId!==perspective.id))fail('CREATOR_ONLY');const content=String(input.content||'').trim().slice(0,300);if(!content)fail('EMPTY_CONTENT');const item={id:input.id,momentId:m.id,perspectiveId:perspective.id,chapterId:m.chapterId,creatorId:user,content,createdAt:input.createdAt||new Date().toISOString()};return versioned('comments',item,existing,event.operationId,input.serverVersion) }
async function deleteItem(user,kind,id) {
  ...
  if(kind==='moments'){for(const row of await list('contributions',{momentId:id}))await write('contributions',row.id,{deleted:true},row);for(const row of await list('comments',{momentId:id}))await write('comments',row.id,{deleted:true},row)}
  if(kind==='contributions'){for(const row of await list('comments',{perspectiveId:id}))await write('comments',row.id,{deleted:true},row)}
...
async function peekInvite(user,event) {
 ...
 // Only basic invitation metadata is public to the token holder. Content is read after joining.
 return withMedia({
  scope,title:scope==='chapter'?target.title:'共同记录一个 Moment',
  inviter:owner&&owner.nickname||'一位朋友',inviterAvatar:owner&&owner.avatar||'',
  cover:scope==='chapter'?target.cover||'':'',startDate:scope==='chapter'?target.startDate||'':'',
  memberCount:scope==='chapter'?(target.memberIds||[]).length:0,
  alreadyMember:scope==='chapter'?(target.memberIds||[]).includes(user):target.creatorId===user||(target.participantIds||[]).includes(user),
...
exports.main=async event=>{try{const user=cloud.getWXContext().OPENID;if(!user)fail('UNAUTHENTICATED');if(event.expectedUserId&&event.expectedUserId!==user)fail('IDENTITY_MISMATCH');const actions={ensureUser:()=>ensureUser(user,event.profile,event.update===true),saveChapter:()=>saveChapter(user,event),saveMoment:()=>saveMoment(user,event),saveContribution:()=>saveContribution(user,event),saveComment:()=>saveComment(user,event),getChapter:()=>chapterSnapshot(event.chapterId,user),getMoment:()=>momentSnapshot(event.momentId,user),deleteChapter:()=>deleteItem(user,'chapters',event.chapterId),deleteMoment:()=>deleteItem(user,'moments',event.momentId),deleteContribution:()=>deleteItem(user,'contributions',event.contributionId),deleteComment:()=>deleteItem(user,'comments',event.commentId),createInvite:()=>createInvite(user,event),peekInvite:()=>peekInvite(user,event),joinInvite:()=>joinInvite(user,event),removeMember:()=>removeMember(user,event),listMine:()=>listMine(user)};if(!actions[event.action])fail('UNKNOWN_ACTION');return {ok:true,protocolVersion:2,userId:user,data:clean(await actions[event.action]())}}catch(e){const detail=String(e.errMsg||e.message||'UNKNOWN_ERROR');let code=e.message||'UNKNOWN_ERROR';if(/DATABASE_COLLECTION_NOT_EXIST|collection.{0,60}(not exist|not found)|集合不存在|-502005/i.test(detail))code='DATABASE_COLLECTION_NOT_EXIST';else if(/DATABASE_PERMISSION_DENIED|permission denied/i.test(detail))code='DATABASE_PERMISSION_DENIED';return {ok:false,error:code,detail}}}
