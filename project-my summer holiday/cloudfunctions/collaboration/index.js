...
exports.main=async event=>{try{const user=cloud.getWXContext().OPENID;if(!user)fail('UNAUTHENTICATED');const actions={ensureUser:()=>ensureUser(user,event.profile,event.update===true),saveChapter:()=>saveChapter(user,event),saveMoment:()=>saveMoment(user,event),saveContribution:()=>saveContribution(user,event),getChapter:()=>chapterSnapshot(event.chapterId,user),getMoment:()=>momentSnapshot(event.momentId,user),deleteChapter:()=>deleteItem(user,'chapters',event.chapterId),deleteMoment:()=>deleteItem(user,'moments',event.momentId),deleteContribution:()=>deleteItem(user,'contributions',event.contributionId),createInvite:()=>createInvite(user,event),peekInvite:()=>peekInvite(user,event),joinInvite:()=>joinInvite(user,event),removeMember:()=>removeMember(user,event),listMine:()=>listMine(user)};if(!actions[event.action])fail('UNKNOWN_ACTION');return {ok:true,data:clean(await actions[event.action]())}}catch(e){const detail=String(e.errMsg||e.message||'UNKNOWN_ERROR');let code=e.message||'UNKNOWN_ERROR';if(/DATABASE_COLLECTION_NOT_EXIST|collection.{0,60}(not exist|not found)|集合不存在|-502005/i.test(detail))code='DATABASE_COLLECTION_NOT_EXIST';else if(/DATABASE_PERMISSION_DENIED|permission denied/i.test(detail))code='DATABASE_PERMISSION_DENIED';return {ok:false,error:code,detail}}}
